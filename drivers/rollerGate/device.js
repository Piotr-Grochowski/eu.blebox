...
module.exports = class rollerGateDevice extends BleBoxDevice {
	...
	async pollBleBox() 
	{
		...
		.then(async result => {
			...
			await this.setCapabilityValue('garagedoor_closed', shutter_closed)
				.catch( err => {
				this.log(err);
				})
			}
	...
	async onCapabilityState( value, opts ) 
  	{
		await this.bbApi.gateControllerSetState(this.getSetting('address'), value)
		.catch(error => {
			this.log(error);
		});
  	}
	...
	async onCapabilityPositionSet( value, opts )
	{
		...
		await this.bbApi.gateControllerSetPosition(this.getSetting('address'), posValue)
		.catch(error => {
			this.log(error);
	    });
	}
	...
	async onCapabilityClosed( value, opts )
	{
		...
		await this.bbApi.gateControllerSetState(this.getSetting('address'), state)
		.catch(error => {
			this.log(error);
	    });
  }
